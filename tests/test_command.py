...
def dehumidifier():
    return LanDevice(id="12345", appliance_type=APPLIANCE_TYPE_DEHUMIDIFIER)


@pytest.fixture(name="aircon")
def aircon():
    return LanDevice(id="12345", appliance_type=APPLIANCE_TYPE_AIRCON)
...
def test_dehumidifier_status(dehumidifier: LanDevice) -> None:
    cmd = dehumidifier.state.refresh_command().finalize()
    assert (
...
def test_dehumidifier_set(dehumidifier: LanDevice) -> None:
    cmd = dehumidifier.state.apply_command().finalize()
    assert (
...
def test_dehumidifier_set_fan_speed(dehumidifier: LanDevice) -> None:
    setattr(dehumidifier.state, "fan_speed", 60)
    cmd = dehumidifier.state.apply_command().finalize()
    assert (
...
def test_dehumidifier_set_mode(dehumidifier: LanDevice) -> None:
    setattr(dehumidifier.state, "mode", 3)
    cmd = dehumidifier.state.apply_command().finalize()
    assert (
...
def test_dehumidifier_set_target_humidity(dehumidifier: LanDevice) -> None:
    setattr(dehumidifier.state, "target_humidity", 45)
    cmd = dehumidifier.state.apply_command().finalize()
    assert (
...
def test_ac_status(aircon: LanDevice) -> None:
    cmd = aircon.state.refresh_command().finalize()
    assert (
...
def test_ac_status_in_sequence(aircon: LanDevice) -> None:
...
def test_aircon_set_fan(aircon: LanDevice) -> None:
    setattr(aircon.state, "beep_prompt", True)
    setattr(aircon.state, "fan_speed", 48)
    cmd = aircon.state.apply_command().finalize()
    assert (
...
def test_aircon_set_mode(aircon: LanDevice) -> None:
    setattr(aircon.state, "beep_prompt", True)
    setattr(aircon.state, "fan_speed", 48)
    setattr(aircon.state, "mode", 2)
    cmd = aircon.state.apply_command().finalize()
    assert (
...
def test_aircon_set_turbo(aircon: LanDevice) -> None:
    setattr(aircon.state, "beep_prompt", True)
    setattr(aircon.state, "turbo", True)
    setattr(aircon.state, "fan_speed", 40)
...
def test_aircon_set_temperature(aircon: LanDevice) -> None:
    setattr(aircon.state, "beep_prompt", True)
    setattr(aircon.state, "turbo", True)
    setattr(aircon.state, "fan_speed", 45)
    setattr(aircon.state, "target_temperature", 20.5)
